    ...
    <div id="content">
        <div class="contentcontainer">
            <div class="add-city-container">
                <div class="heading">Edit Area</div>
                <form class="add-city-form form" action="/admin-dashboard/edit-subcategory/${subcat.id}/add" method="POST" enctype="multipart/form-data">
                    <div class="form-group mb-5">
                        <label for="cityId">Select City</label>
                        <select class="form-select" id="cityId" name="cityId" aria-label="Select Category" required>
                            <option value="" disabled>Select Area</option>
                            <c:forEach items="${category}" var="category">
                                <c:choose>
                                    <c:when test="${category.id == subcat.category.id}">
                    ...
                    <div class="form-group mb-5">
                        <label for="cityName">Area Name</label>
                        <input type="text" class="form-control" id="cityName" name="cityName" aria-describedby="emailHelp" placeholder="City Name" value="${subcat.name}" required>
                    </div>
                    <div class="form-group mb-5">
                        <label for="cityDesc">Area Description</label>
                        <div id="">
                            <textarea class="form-control"  id="cityDesc" name="cityDesc" rows="3" placeholder="Description" required>${subcat.description}</textarea>
                        </div>
                    </div>
